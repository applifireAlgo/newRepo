Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseRespondentMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CaseRespondentMainController',
     init: function() {
          var form = this.view.down('#CaseRespondentForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CaseRespondentTree');
               ...
               var compRef = this.view.down('#CaseRespondentTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#CaseRespondentGrid');
          var tree = this.view.down('#CaseRespondentTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#CaseRespondent');
          var grid = this.view.down('#CaseRespondentGrid');
          var tree = this.view.down('#CaseRespondentTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseRespondentForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CaseRespondentTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var CaseRespondent = this.view.down('#CaseRespondentForm');
               CaseRespondent.reset();
               var gridPanel = this.view.down('#CaseRespondentGrid');
          ...
          var formPanel = this.getView().up().down('#CaseRespondentForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseRespondentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseRespondent').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseRespondentGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseTypeMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CaseTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CaseTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CaseTypeGrid');
               var tree = this.view.down('#CaseTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CaseTypeForm');
          var grid = this.view.down('#CaseTypeGrid');
          var tree = this.view.down('#CaseTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CaseTypeGrid');
               ...
               var formPanel = this.getView().up().down('#CaseTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseTypeGrid').store;
          gridStore.removeAll();

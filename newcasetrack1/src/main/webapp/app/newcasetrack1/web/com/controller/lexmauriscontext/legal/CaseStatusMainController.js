Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseStatusMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CaseStatusMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CaseStatusTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseStatusForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CaseStatusGrid');
               var tree = this.view.down('#CaseStatusTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CaseStatusForm');
          var grid = this.view.down('#CaseStatusGrid');
          var tree = this.view.down('#CaseStatusTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CaseStatusGrid');
               ...
               var formPanel = this.getView().up().down('#CaseStatusForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseStatusTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseStatusForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseStatusGrid').store;
          gridStore.removeAll();

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.StatuteMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.StatuteMainController',
     ...
     init: function() {
          ...
          this.litCategoryIdLoad();
     },
     litCategoryIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#litCategoryId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'litCategoryId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#StatuteTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#StatuteForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#StatuteGrid');
               var tree = this.view.down('#StatuteTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#StatuteForm');
          var grid = this.view.down('#StatuteGrid');
          var tree = this.view.down('#StatuteTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#StatuteGrid');
               ...
               var formPanel = this.getView().up().down('#StatuteForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#StatuteTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#StatuteForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#StatuteGrid').store;
          gridStore.removeAll();

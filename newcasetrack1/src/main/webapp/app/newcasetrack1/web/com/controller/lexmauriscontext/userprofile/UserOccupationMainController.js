Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.userprofile.UserOccupationMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.UserOccupationMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#UserOccupationTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#UserOccupationForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#UserOccupationGrid');
               var tree = this.view.down('#UserOccupationTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#UserOccupationForm');
          var grid = this.view.down('#UserOccupationGrid');
          var tree = this.view.down('#UserOccupationTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#UserOccupationGrid');
               ...
               var formPanel = this.getView().up().down('#UserOccupationForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#UserOccupationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#UserOccupationForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#UserOccupationGrid').store;
          gridStore.removeAll();

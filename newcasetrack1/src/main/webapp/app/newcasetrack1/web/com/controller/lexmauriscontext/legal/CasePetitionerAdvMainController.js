Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CasePetitionerAdvMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CasePetitionerAdvMainController',
     ...
     caseIdLoad: function() {
          ...
          Ext.Ajax.request({
               url: 'secure/CaseInformation/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#caseId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'caseId', response.responseText);
     ...
     casePetAdvLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#casePetAdv');
                    scope.sender.controller.assignAllComboData(scope.sender, 'casePetAdv', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#casePetAdv');
     ...
     onCasePetAdvChange: function(me, newValue, oldValue, eOpts) {
          this.onCasePetAdvChangeAdvCatId(me, newValue, oldValue, eOpts);
     },
     onCaseIdChange: function(me, newValue, oldValue, eOpts) {
          this.onCaseIdChangeAdvCatId(me, newValue, oldValue, eOpts);
     },
     onCasePetAdvChangeAdvCatId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CasePetitionerAdv/findByCasePetAdv',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var advCatIdCombo = scope.element.up('form').down('#advCatId');
                    scope.sender.controller.assignComboData(advCatIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, advCatIdCombo);
               },
               failure: function(response, scope) {
     ...
     onCaseIdChangeAdvCatId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CasePetitionerAdv/findByCaseId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CasePetitionerAdvTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CasePetitionerAdvForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CasePetitionerAdvGrid');
               var tree = this.view.down('#CasePetitionerAdvTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CasePetitionerAdvForm');
          var grid = this.view.down('#CasePetitionerAdvGrid');
          var tree = this.view.down('#CasePetitionerAdvTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CasePetitionerAdvGrid');
               ...
               var formPanel = this.getView().up().down('#CasePetitionerAdvForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CasePetitionerAdvTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CasePetitionerAdvForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CasePetitionerAdvGrid').store;
          gridStore.removeAll();

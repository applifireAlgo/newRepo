Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseInformationMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CaseInformationMainController',
     init: function() {
          var form = this.view.down('#CaseInformationForm');
          ...
          this.caseStatusIdLoad();
          this.forumIdLoad();
     },
     caseStatusIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/CaseStatus/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#caseStatusId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'caseStatusId', response.responseText);
     ...
     onForumIdChange: function(me, newValue, oldValue, eOpts) {
          this.onForumIdChangeCaseTypeId(me, newValue, oldValue, eOpts);
     },
     onForumIdChangeCaseTypeId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CaseType/findByForumId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var caseTypeIdCombo = scope.element.up('form').down('#caseTypeId');
                    scope.sender.controller.assignComboData(caseTypeIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, caseTypeIdCombo);
               },
               failure: function(response, scope) {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CaseInformationTree');
               ...
               var compRef = this.view.down('#CaseInformationTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#CaseInformationGrid');
          var tree = this.view.down('#CaseInformationTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#CaseInformation');
          var grid = this.view.down('#CaseInformationGrid');
          var tree = this.view.down('#CaseInformationTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseInformationForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CaseInformationTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var CaseInformation = this.view.down('#CaseInformationForm');
               CaseInformation.reset();
               var gridPanel = this.view.down('#CaseInformationGrid');
          ...
          var formPanel = this.getView().up().down('#CaseInformationForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseInformationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseInformation').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseInformationGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

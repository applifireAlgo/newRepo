Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.ForumMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ForumMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ForumTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ForumForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ForumGrid');
               var tree = this.view.down('#ForumTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ForumForm');
          var grid = this.view.down('#ForumGrid');
          var tree = this.view.down('#ForumTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ForumGrid');
               ...
               var formPanel = this.getView().up().down('#ForumForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ForumTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ForumForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ForumGrid').store;
          gridStore.removeAll();

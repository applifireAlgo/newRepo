Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseJudgeMapMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CaseJudgeMapMainController',
     init: function() {
          var form = this.view.down('#CaseJudgeMapForm');
     ...
     judgeIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/JudgeRegistration/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#judgeId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'judgeId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#judgeId');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CaseJudgeMapTree');
               ...
               var compRef = this.view.down('#CaseJudgeMapTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#CaseJudgeMapGrid');
          var tree = this.view.down('#CaseJudgeMapTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#CaseJudgeMap');
          var grid = this.view.down('#CaseJudgeMapGrid');
          var tree = this.view.down('#CaseJudgeMapTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseJudgeMapForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CaseJudgeMapTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var CaseJudgeMap = this.view.down('#CaseJudgeMapForm');
               CaseJudgeMap.reset();
               var gridPanel = this.view.down('#CaseJudgeMapGrid');
          ...
          var formPanel = this.getView().up().down('#CaseJudgeMapForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseJudgeMapTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseJudgeMap').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseJudgeMapGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

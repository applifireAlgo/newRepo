Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.DailyCaseTrackerMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.DailyCaseTrackerMainController',
     init: function() {
          var form = this.view.down('#DailyCaseTrackerForm');
     ...
     forumIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#forumId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'forumId', response.responseText);
     ...
     onForumIdChange: function(me, newValue, oldValue, eOpts) {
          this.onForumIdChangeCaseId(me, newValue, oldValue, eOpts);
          this.onForumIdChangeJudgeRegId(me, newValue, oldValue, eOpts);
     },
     onForumIdChangeCaseId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/CaseInformation/findByForumId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var caseIdCombo = scope.element.up('form').down('#caseId');
                    scope.sender.controller.assignComboData(caseIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, caseIdCombo);
               },
               failure: function(response, scope) {
     ...
     onForumIdChangeJudgeRegId: function(me, newValue, oldValue, eOpts) {
          var scope = this.getView();
          Ext.Ajax.request({
               async: false,
               url: 'secure/JudgeRegistration/findByForumId',
               method: 'POST',
               jsonData: {
                    findKey: newValue
               },
               ...
               element: me,
               success: function(response, scope) {
                    var judgeRegIdCombo = scope.element.up('form').down('#judgeRegId');
                    scope.sender.controller.assignComboData(judgeRegIdCombo, response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, judgeRegIdCombo);
               },
               failure: function(response, scope) {
                    scope.sender.controller.addCommunicationLog(response, false);
               }
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#DailyCaseTrackerTree');
               ...
               var compRef = this.view.down('#DailyCaseTrackerTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#DailyCaseTrackerGrid');
          var tree = this.view.down('#DailyCaseTrackerTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#DailyCaseTracker');
          var grid = this.view.down('#DailyCaseTrackerGrid');
          var tree = this.view.down('#DailyCaseTrackerTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DailyCaseTrackerForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#DailyCaseTrackerTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var DailyCaseTracker = this.view.down('#DailyCaseTrackerForm');
               DailyCaseTracker.reset();
               var gridPanel = this.view.down('#DailyCaseTrackerGrid');
          ...
          var formPanel = this.getView().up().down('#DailyCaseTrackerForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#DailyCaseTrackerTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DailyCaseTracker').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DailyCaseTrackerGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.JudgeRegistrationMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.JudgeRegistrationMainController',
     init: function() {
          var form = this.view.down('#JudgeRegistrationForm');
     ...
     jContactIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#jContactId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'jContactId', response.responseText);
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#JudgeRegistrationTree');
               ...
               var compRef = this.view.down('#JudgeRegistrationTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#JudgeRegistrationGrid');
          var tree = this.view.down('#JudgeRegistrationTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#JudgeRegistration');
          var grid = this.view.down('#JudgeRegistrationGrid');
          var tree = this.view.down('#JudgeRegistrationTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#JudgeRegistrationForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#JudgeRegistrationTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var JudgeRegistration = this.view.down('#JudgeRegistrationForm');
               JudgeRegistration.reset();
               var gridPanel = this.view.down('#JudgeRegistrationGrid');
          ...
          var formPanel = this.getView().up().down('#JudgeRegistrationForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#JudgeRegistrationTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#JudgeRegistration').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#JudgeRegistrationGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

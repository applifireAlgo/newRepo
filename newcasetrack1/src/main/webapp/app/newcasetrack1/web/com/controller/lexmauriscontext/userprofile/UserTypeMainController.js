Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.userprofile.UserTypeMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.UserTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#UserTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#UserTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#UserTypeGrid');
               var tree = this.view.down('#UserTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#UserTypeForm');
          var grid = this.view.down('#UserTypeGrid');
          var tree = this.view.down('#UserTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#UserTypeGrid');
               ...
               var formPanel = this.getView().up().down('#UserTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#UserTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#UserTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#UserTypeGrid').store;
          gridStore.removeAll();

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.LitigationCategoryMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.LitigationCategoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#LitigationCategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#LitigationCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#LitigationCategoryGrid');
               var tree = this.view.down('#LitigationCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#LitigationCategoryForm');
          var grid = this.view.down('#LitigationCategoryGrid');
          var tree = this.view.down('#LitigationCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#LitigationCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#LitigationCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#LitigationCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#LitigationCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#LitigationCategoryGrid').store;
          gridStore.removeAll();

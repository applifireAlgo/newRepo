Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.defaultcontext.defaultdomain.StudentMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.StudentMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#StudentTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#StudentForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#StudentGrid');
               var tree = this.view.down('#StudentTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#StudentForm');
          var grid = this.view.down('#StudentGrid');
          var tree = this.view.down('#StudentTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#StudentGrid');
               ...
               var formPanel = this.getView().up().down('#StudentForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#StudentTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#StudentForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#StudentGrid').store;
          gridStore.removeAll();

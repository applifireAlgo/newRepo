Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CaseRespondentAdvMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CaseRespondentAdvMainController',
     init: function() {
          var form = this.view.down('#CaseRespondentAdvForm');
     ...
     advCatIdLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               url: 'secure/AdvocateCategory/findAll',
               method: 'GET',
               sender: scope,
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#advCatId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'advCatId', response.responseText);
     ...
     advocateIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#advocateId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'advocateId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#advocateId');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CaseRespondentAdvTree');
               ...
               var compRef = this.view.down('#CaseRespondentAdvTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#CaseRespondentAdvGrid');
          var tree = this.view.down('#CaseRespondentAdvTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#CaseRespondentAdv');
          var grid = this.view.down('#CaseRespondentAdvGrid');
          var tree = this.view.down('#CaseRespondentAdvTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CaseRespondentAdvForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CaseRespondentAdvTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var CaseRespondentAdv = this.view.down('#CaseRespondentAdvForm');
               CaseRespondentAdv.reset();
               var gridPanel = this.view.down('#CaseRespondentAdvGrid');
          ...
          var formPanel = this.getView().up().down('#CaseRespondentAdvForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CaseRespondentAdvTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CaseRespondentAdv').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CaseRespondentAdvGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.CasePetitionersMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.CasePetitionersMainController',
     init: function() {
          var form = this.view.down('#CasePetitionersForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#CasePetitionersTree');
               ...
               var compRef = this.view.down('#CasePetitionersTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#CasePetitionersGrid');
          var tree = this.view.down('#CasePetitionersTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#CasePetitioners');
          var grid = this.view.down('#CasePetitionersGrid');
          var tree = this.view.down('#CasePetitionersTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CasePetitionersForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#CasePetitionersTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var CasePetitioners = this.view.down('#CasePetitionersForm');
               CasePetitioners.reset();
               var gridPanel = this.view.down('#CasePetitionersGrid');
          ...
          var formPanel = this.getView().up().down('#CasePetitionersForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CasePetitionersTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CasePetitioners').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CasePetitionersGrid').store;
          gridStore.removeAll();
          gridStore.add(data);

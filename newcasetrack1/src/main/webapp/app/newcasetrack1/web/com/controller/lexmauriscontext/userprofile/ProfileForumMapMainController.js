Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.userprofile.ProfileForumMapMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ProfileForumMapMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ProfileForumMapTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProfileForumMapForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ProfileForumMapGrid');
               var tree = this.view.down('#ProfileForumMapTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ProfileForumMapForm');
          var grid = this.view.down('#ProfileForumMapGrid');
          var tree = this.view.down('#ProfileForumMapTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ProfileForumMapGrid');
               ...
               var formPanel = this.getView().up().down('#ProfileForumMapForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ProfileForumMapTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProfileForumMapForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProfileForumMapGrid').store;
          gridStore.removeAll();

Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.LitigationSubCategoryMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.LitigationSubCategoryMainController',
     ...
     init: function() {
          ...
          this.litigationCategoryIdLoad();
     },
     litigationCategoryIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#litigationCategoryId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'litigationCategoryId', response.responseText);
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#LitigationSubCategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#LitigationSubCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#LitigationSubCategoryGrid');
               var tree = this.view.down('#LitigationSubCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#LitigationSubCategoryForm');
          var grid = this.view.down('#LitigationSubCategoryGrid');
          var tree = this.view.down('#LitigationSubCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#LitigationSubCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#LitigationSubCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#LitigationSubCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#LitigationSubCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#LitigationSubCategoryGrid').store;
          gridStore.removeAll();

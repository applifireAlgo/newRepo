Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.userprofile.ProfileLitSubCategoryMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ProfileLitSubCategoryMainController',
     ...
     init: function() {
          ...
          this.litCatIdLoad();
          this.litSubCatIdLoad();
          this.regIdLoad();
     },
     litCatIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#litCatId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'litCatId', response.responseText);
     ...
     litSubCatIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#litSubCatId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'litSubCatId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#litSubCatId');
     ...
     regIdLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#regId');
                    scope.sender.controller.assignAllComboData(scope.sender, 'regId', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#regId');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ProfileLitSubCategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ProfileLitSubCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ProfileLitSubCategoryGrid');
               var tree = this.view.down('#ProfileLitSubCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ProfileLitSubCategoryForm');
          var grid = this.view.down('#ProfileLitSubCategoryGrid');
          var tree = this.view.down('#ProfileLitSubCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ProfileLitSubCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#ProfileLitSubCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ProfileLitSubCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ProfileLitSubCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ProfileLitSubCategoryGrid').store;
          gridStore.removeAll();

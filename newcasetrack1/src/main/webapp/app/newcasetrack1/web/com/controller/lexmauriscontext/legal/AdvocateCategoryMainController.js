Ext.define('Newcasetrack1.newcasetrack1.web.com.controller.lexmauriscontext.legal.AdvocateCategoryMainController', {
     extend: 'Newcasetrack1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AdvocateCategoryMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AdvocateCategoryTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#AdvocateCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#AdvocateCategoryGrid');
               var tree = this.view.down('#AdvocateCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#AdvocateCategoryForm');
          var grid = this.view.down('#AdvocateCategoryGrid');
          var tree = this.view.down('#AdvocateCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#AdvocateCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#AdvocateCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#AdvocateCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#AdvocateCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AdvocateCategoryGrid').store;
          gridStore.removeAll();

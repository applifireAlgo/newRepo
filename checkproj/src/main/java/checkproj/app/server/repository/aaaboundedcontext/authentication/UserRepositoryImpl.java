package checkproj.app.server.repository.aaaboundedcontext.authentication;
...
public class UserRepositoryImpl extends SearchInterfaceImpl implements UserRepository<User> {
    ...
    public List<User> findAll() throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            java.util.List<checkproj.app.shared.aaaboundedcontext.authentication.User> query = emanager.createQuery("select u from User u where u.systemInfo.activeStatus=1").getResultList();
            return query;
        } catch (javax.persistence.PersistenceException e) {
    ...
    public List<User> save(List<User> entity) throws SpartanPersistenceException, SpartanConstraintViolationException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (int i = 0; i < entity.size(); i++) {
                checkproj.app.shared.aaaboundedcontext.authentication.User obj = entity.get(i);
    ...
    public void delete(String id) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            checkproj.app.shared.aaaboundedcontext.authentication.User s = emanager.find(checkproj.app.shared.aaaboundedcontext.authentication.User.class, id);
            emanager.remove(s);
        } catch (javax.persistence.PersistenceException e) {
    ...
    public void deletePassRecovery(List<PassRecovery> passrecovery) throws SpartanPersistenceException {
        try {
            javax.persistence.EntityManager emanager = emfResource.getResource();
            for (checkproj.app.shared.aaaboundedcontext.authentication.PassRecovery _passrecovery : passrecovery) {
                checkproj.app.shared.aaaboundedcontext.authentication.PassRecovery s = emanager.find(checkproj.app.shared.aaaboundedcontext.authentication.PassRecovery.class, _passrecovery.getPassRecoveryId());
                emanager.remove(s);
            }
        } catch (javax.persistence.PersistenceException e) {
            throw new SpartanPersistenceException("Error in deleting entity", e);
    ...
    public List<User> findByUserAccessLevelId(String userAccessLevelId) throws Exception, SpartanPersistenceException {
        try {
            ...
            java.util.List<checkproj.app.shared.aaaboundedcontext.authentication.User> listOfUser = query.getResultList();
            return listOfUser;
        } catch (javax.persistence.PersistenceException e) {
            throw new com.athena.framework.server.exception.repository.SpartanPersistenceException("Error in executing query", e);
        }
    ...
    public User findById(String userId) throws Exception, SpartanPersistenceException {
        try {
            ...
            javax.persistence.Query query = emanager.createNamedQuery("User.findById");
            query.setParameter("userId", userId);
            checkproj.app.shared.aaaboundedcontext.authentication.User listOfUser = (checkproj.app.shared.aaaboundedcontext.authentication.User) query.getSingleResult();
            return listOfUser;
        } catch (javax.persistence.PersistenceException e) {
            throw new com.athena.framework.server.exception.repository.SpartanPersistenceException("Error in executing query", e);
        }
